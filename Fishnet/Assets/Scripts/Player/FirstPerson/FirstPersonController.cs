namespace EasyPeasyFirstPersonController
{
    ...
    public partial class FirstPersonController : NetworkBehaviour
    {
        ...
        public float minCameraOffset = -90f;
        public float maxCameraOffset = 45f;
        ...
        {
            float mouseX = input.lookInput.x * mouseSensitivity * 0.1f;
            float mouseY = input.lookInput.y * mouseSensitivity * 0.1f;

            transform.Rotate(Vector3.up * mouseX);

            xRotation -= mouseY;

            xRotation = Mathf.Clamp(xRotation, minCameraOffset, maxCameraOffset);
